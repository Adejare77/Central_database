...
class Filter(CreateClassTable):
    def __init__(self, id, db, tables, columns=None) -> None:
        ...
        self.tbl_headers = self.__table_headers(self.tables, self.columns)

    def __table_headers(self, tables, columns=None) -> list:
        if columns:
            return columns
    ...
    def __multiple_tables(self, tbl_clss, join_type, conditions):
        session = self.Session()
        data = []
        if not join_type:  # defaults to using join
            query = session.query(*tbl_clss).all()

        else:
            if join_type == "rightjoin":
                tbl_clss[0], tbl_clss[1] = tbl_clss[1], tbl_clss[0]
            if join_type == "rightjoin" or join_type == "leftjoin":
                join_type = "outerjoin"
    ...
    def join_conditions(self):
        ...
        for i in range(1, len(self.tables)):
            col1 = self.__table_headers(self.tables[i-1], None)
            col2 = self.__table_headers(self.tables[i], None)
